...
__contract__(
  ...
  requires(buflen >= 0 && buflen <= (POLY_UNIFORM_NBLOCKS * STREAM128_BLOCKBYTES) && buflen % 3 == 0)
...
{
  int ctr;
  int buflen = POLY_UNIFORM_NBLOCKS * STREAM128_BLOCKBYTES;
  ...
  __loop__(
    ...
    invariant(state.pos >= 0 && state.pos <= SHAKE128_RATE)
  )
  {
