...
__contract__(
  requires(arrays_no_alias(sig, MLDSA_CRYPTO_BYTES, mu, MLDSA_CRHBYTES,
                           rhoprime, MLDSA_CRHBYTES, challenge_bytes, MLDSA_CTILDEBYTES))
  requires(objs_no_alias(mat, s1, s2, t0, h, z, w1, w0, cp))
  requires((void*) y == (void*) h)
  ...
  assigns_arrs(sig, MLDSA_CRYPTO_BYTES, challenge_bytes, MLDSA_CTILDEBYTES)
  assigns_objs(h, y, z, w1, w0, cp)
