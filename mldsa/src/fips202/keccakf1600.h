...
__contract__(
    ...
    requires(slices_no_alias(state, sizeof(uint64_t) * MLD_KECCAK_LANES))
    requires(slices_no_alias(data, length))
    assigns_slices(data, length)
);
...
__contract__(
    ...
    requires(slices_no_alias(state, sizeof(uint64_t) * MLD_KECCAK_LANES))
    requires(slices_no_alias(data, length))
    assigns_slices(state, sizeof(uint64_t) * MLD_KECCAK_LANES)
);
...
__contract__(
    requires(0 <= offset && offset <= MLD_KECCAK_LANES * sizeof(uint64_t) &&
	     0 <= length && length <= MLD_KECCAK_LANES * sizeof(uint64_t) - offset)
    requires(slices_no_alias(state, sizeof(uint64_t) * MLD_KECCAK_LANES))
    requires(slices_no_alias(data0, length))
    requires(slices_no_alias(data1, length))
    requires(slices_no_alias(data2, length))
    requires(slices_no_alias(data3, length))
    assigns_slices(data0, length)
    assigns_slices(data1, length)
    assigns_slices(data2, length)
    assigns_slices(data3, length)
);
...
__contract__(
    requires(0 <= offset && offset <= MLD_KECCAK_LANES * sizeof(uint64_t) &&
	     0 <= length && length <= MLD_KECCAK_LANES * sizeof(uint64_t) - offset)
    requires(slices_no_alias(state, sizeof(uint64_t) * MLD_KECCAK_LANES * MLD_KECCAK_WAY))
    requires(slices_no_alias(data0, length))
    /* Case 1: all input buffers are distinct; Case 2: All input buffers are the same */
    requires((data0 == data1 &&
              data0 == data2 &&
              data0 == data3) ||
	     (slices_no_alias(data1, length) &&
              slices_no_alias(data2, length) &&
              slices_no_alias(data3, length)))
    assigns_slices(state, sizeof(uint64_t) * MLD_KECCAK_LANES * MLD_KECCAK_WAY)
);
