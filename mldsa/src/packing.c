                        ...
                        const uint8_t sk[CRYPTO_SECRETKEYBYTES])
{
  ...
  /* Copy s1 and convert to NTT representation */
  s1_ntt = s1;
  mld_polyvecl_ntt(&s1_ntt);

  /* Matrix-vector multiplication: t = A * s1 (in NTT domain) */
  mld_polyvec_matrix_pointwise_montgomery(&t, mat, &s1_ntt);

  /* Reduce modulo field */
  mld_polyveck_reduce(&t);

  /* Take t out of NTT representation */
