...
MLD_HOST_SUPPORTS_AVX2 := $(call check_host_feature,avx2,cat /proc/cpuinfo)
MLD_HOST_SUPPORTS_SSE2 := $(call check_host_feature,sse2,cat /proc/cpuinfo)
MLD_HOST_SUPPORTS_BMI2 := $(call check_host_feature,bmi2,cat /proc/cpuinfo)
else ifeq ($(HOST_PLATFORM),Darwin-x86_64)
# macOS: Use sysctl
MLD_HOST_SUPPORTS_AVX2 := $(call check_host_feature,AVX2,sysctl -n machdep.cpu.leaf7_features)
MLD_HOST_SUPPORTS_SSE2 := $(call check_host_feature,SSE2,sysctl -n machdep.cpu.features)
MLD_HOST_SUPPORTS_BMI2 := $(call check_host_feature,BMI2,sysctl -n machdep.cpu.leaf7_features)
else ifneq ($(CROSS_PREFIX),)
# Cross-compilation: assume all features are supported
MLD_HOST_SUPPORTS_AVX2 := 1
MLD_HOST_SUPPORTS_SSE2 := 1
MLD_HOST_SUPPORTS_BMI2 := 1
else
# Other platforms: assume no support
MLD_HOST_SUPPORTS_AVX2 := 0
MLD_HOST_SUPPORTS_SSE2 := 0
MLD_HOST_SUPPORTS_BMI2 := 0
...
MLD_HOST_SUPPORTS_SHA3 := $(call check_host_feature,sha3,cat /proc/cpuinfo)
else ifeq ($(HOST_PLATFORM),Darwin-arm64)
# macOS: Use sysctl to check for SHA3 support
MLD_HOST_SUPPORTS_SHA3 := $(call check_host_feature,1,sysctl -n hw.optional.armv8_2_sha3)
else ifneq ($(CROSS_PREFIX),)
# Cross-compilation: assume all features are supported
MLD_HOST_SUPPORTS_SHA3 := 1
else
# Other platforms: assume no support
MLD_HOST_SUPPORTS_SHA3 := 0
...
ifeq ($(MLD_COMPILER_SUPPORTS_AVX2)$(MLD_HOST_SUPPORTS_AVX2),11)
CFLAGS += -mavx2
endif

ifeq ($(MLD_COMPILER_SUPPORTS_BMI2)$(MLD_HOST_SUPPORTS_BMI2),11)
CFLAGS += -mbmi2
endif
endif # x86_64
...
ifeq ($(MLD_COMPILER_SUPPORTS_SHA3)$(MLD_HOST_SUPPORTS_SHA3),11)
